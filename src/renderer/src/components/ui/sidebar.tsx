...
function SidebarProvider({
  ...
  const [_width, _setWidth] = React.useState(widthProp ?? 256);
...
function SidebarMenuAction({
  ...
  return (
    <Comp
      ...
      className={cn(
        'absolute top-1/2 right-1 flex aspect-square w-5 -translate-y-1/2 items-center justify-center rounded-xl p-0 text-sidebar-foreground ring-sidebar-ring outline-hidden transition-[opacity,transform] group-data-[collapsible=icon]:hidden peer-hover/menu-button:text-sidebar-accent-foreground after:absolute after:-inset-2 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground hover:opacity-100! focus-visible:ring-2 md:after:hidden [&>svg]:size-4 [&>svg]:shrink-0',
        showOnHover &&
          'group-focus-within/menu-item:opacity-50 group-hover/menu-item:opacity-50 peer-data-active/menu-button:text-sidebar-accent-foreground aria-expanded:opacity-100 md:opacity-0',
        className,
      )}
      {...props}
    />
...
function SidebarResizeHandle({ className, ...props }: React.ComponentProps<'button'>) {
